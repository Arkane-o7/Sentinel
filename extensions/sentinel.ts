...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event, ctx) => {
    let r;
    try {
      r = await assessAction({ tool: event.toolName, input: event.input, cwd: ctx.cwd, agent: "pi", sessionId: sessionId(ctx), context: context(ctx) }, { signal: ctx.signal });
      ...
      return failClosed() ? { block: true, reason: `sentinel unavailable: ${(err as Error).message}` } : undefined;
    }
    ...
    const ok = await ctx.ui.confirm("sentinel: approve this tool call?", r.message);
