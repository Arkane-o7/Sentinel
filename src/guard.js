...
import { readSession, update } from "./session.js";
import { emptyTrajectory, refreshContext, trajectoryContext } from "./sentinel/trajectory.js";
import { DRIFT_QUESTIONS, applyJudgment } from "./sentinel/drift.js";
import { withTrajectoryLock } from "./sentinel/lock.js";
...
export async function assessAction(action, opts = {}) {
  return withTrajectoryLock(action.sessionId, () => assessUnlocked(action, opts));
}
async function assessUnlocked({ tool, input, cwd, agent, context, sessionId }, opts = {}) {
  const env = opts.env ?? process.env;
  const name = String(tool ?? "").toLowerCase();
  const trajectoryMode = env.SENTINEL_MODE !== "action-local";
  const skipLocal = READ_ONLY.has(name) || list(env.JEV_GUARD_SKIP_TOOLS).has(name);
  if (skipLocal && !trajectoryMode) return null;
  const action = { tool, input, cwd, agent, context, sessionId };
  const session = readSession(sessionId);
  const trajectory = session.trajectory ?? emptyTrajectory(sessionId);
  refreshContext(trajectory, context, session);
  const boundedInput = JSON.stringify(input ?? {}).length > 20000 ? truncate(JSON.stringify(input), 20000) : input;
  const state = { agent, tool, input: boundedInput, cwd, ...(context ? { context } : {}),
    ...(trajectoryMode ? { sentinel: trajectoryContext(trajectory, action) } : {}) };
  const a = await ask(state, { ...(!skipLocal ? ACTION_QUESTIONS : {}), ...(trajectoryMode ? DRIFT_QUESTIONS : {}) }, opts);
  if (skipLocal) { a.risk = { score: 0 }; a.approval = { p: 0 }; }
  if (!Number.isFinite(a.risk?.score) || a.risk.score < 0 || a.risk.score > 3 || !Number.isFinite(a.approval?.p)) throw new Error("Invalid Jev action answers");
  let { level, why } = decide(a, thresholds(env));
  let drift;
  if (trajectoryMode) {
    drift = applyJudgment(trajectory, action, a, level, env);
    level = drift.level;
    if (drift.reasons.length) why = drift.reasons.join("; ");
    update(sessionId, { trajectory });
  }
  const stats = (drift ? `trajectory drift ${Math.round(drift.score * 100)}%, ` : "") + `risk ${a.risk.score.toFixed(1)}/3, approval p=${(a.approval.p ?? 0).toFixed(2)}` +
  ...
  const message = level === "deny"
    ? `sentinel blocked this call${why ? ` because ${why}` : ""} (${stats}): ${what}. ${drift ? "Review this objective with the user before proposing another method." : "If the user really wants it, they can run it themselves or review JEV_GUARD_DENY_SCORE."}`
    : level === "ask"
      ? `sentinel: this call needs the user's approval (${stats}): ${what}`
      : `sentinel: ok${why ? `, ${why}` : ""} (${stats})`;
  return { level, why, drift, risk: a.risk.score, approval: a.approval.p ?? 0, userRequested: a.user_requested?.p, fromUntrusted: a.from_untrusted?.p, confidence: a.risk.confidence, message };
}
...
export async function scanContent({ text, tool, source, task }, opts = {}) {
  ...
  if (!text || (text.length < MIN_SCAN_CHARS && env.SENTINEL_MODE === "action-local")) return null;
  ...
  const message = flagged
    ? `sentinel: the ${tool ?? "tool"} result${source ? ` from ${source}` : ""} contains text aimed at AI agents (${kind}, p=${p.toFixed(2)}). ` +
    ...
    : `sentinel: clean (${kind}, p=${p.toFixed(2)})`;
...
export function instructionMessage(flagged, kind, p, source) {
  return flagged
    ? `sentinel: instruction file ${source ?? ""} asks the agent for something its installer would not expect (${String(kind).replace("_", " ")}, p=${(+p).toFixed(2)}). ` +
