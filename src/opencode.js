...
// lets sentinel auto-approve the safe calls and keep the prompt for the risky ones; tool.execute.after flags results.
...
export const JevGuard = async ({ client, directory }) => {
  const toast = (message, variant = "warning") =>
    client?.tui?.showToast?.({ body: { title: "sentinel", message, variant, duration: 8000 } }).catch(() => {});
  const failOpen = (err) => {
    toast(err.message, "error");
    if (failClosed()) throw new Error(`sentinel unavailable: ${err.message}`);
    return null;
  };
  ...
  return {
    "tool.execute.before": async (input, output) => {
      const r = await assessAction({ tool: input.tool, input: output.args, cwd: directory, agent: "opencode", sessionId: input.sessionID, context: await context(input.sessionID) }).catch(failOpen);
      ...
      if (r.level === "deny" || r.drift?.level === "ask") throw new Error(r.message);
    ...
    "permission.ask": async (input, output) => {
      ...
      const r = await assessAction({ tool: input.type, input: args, cwd: directory, agent: "opencode", sessionId: input.sessionID, context: await context(input.sessionID) }).catch(failOpen);
      if (!r) return;
