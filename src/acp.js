...
export function runProxy(cmd, args, { stdin = process.stdin, stdout = process.stdout, env = process.env, fetchImpl, onExit = (c) => process.exit(c) } = {}) {
  ...
  const warn = (err) => process.stderr.write(`sentinel: ${err.message ?? err}\n`);
  ...
  async function guardRequest(msg) {
    ...
    else if (msg.method === "fs/read_text_file") { tool = "Read"; input = { file_path: p.path }; kind = "read"; }
    ...
    try { r = await assessAction({ tool, input, cwd: p.cwd, agent: "acp", sessionId: p.sessionId, context }, opts); }
    catch (err) { warn(err); if (!failClosed(env)) return null; r = { level: "deny", message: `sentinel unavailable: ${err.message}` }; }
    ...
    if (r.level === "ask") {
      const res = await askClient("session/request_permission", {
        ...
        toolCall: { toolCallId: `sentinel-${seq + 1}`, title: `sentinel: ${tool} ${preview(input)}`, kind, status: "pending", rawInput: input },
  ...
  async function flagContent(msg, method, req) {
    ...
    if (typeof text !== "string" || (text.length < MIN_SCAN_CHARS && env.SENTINEL_MODE === "action-local")) return;
