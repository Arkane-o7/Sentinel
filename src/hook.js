...
export async function handleHook(input, { agent, env = process.env, fetchImpl } = {}) {
  ...
  const assess = async (tool, toolInput, intent) => {
    const r = await assessAction({ tool, input: toolInput, cwd: input.cwd, agent, sessionId, context: ctxOf(intent) }, opts);
  ...
  if (PROMPT_EVENTS.has(event)) {
    ...
    const note = `sentinel: ${pending.length} instruction file(s) in this session contain unexpected instructions — ` +
  ...
  if (SESSION_EVENTS.has(event)) {
    ...
    const note = `sentinel: ${flagged.length} project instruction file(s) contain unexpected instructions — ` +
  ...
  if (event === "PreToolUse" || event === "PermissionRequest") {
    ...
    if (agent === "codex" && r.drift?.level === "ask") return decision("deny");
  ...
  if (event === "BeforeTool") {
    const r = await assess(input.tool_name, input.tool_input);
    if (!r || r.level === "allow") return null;
    if (r.level === "deny" || r.drift?.level === "ask") return { decision: "deny", reason: r.message };
  ...
  if (CURSOR_PERMISSION_EVENTS.has(event)) {
    ...
    if (r.level === "ask" && event === "preToolUse") return { permission: r.drift?.level === "ask" ? "deny" : "allow" };  // ask is accepted but not enforced there
...
export async function main(argv = process.argv.slice(2), stdin = process.stdin, stdout = process.stdout, env = process.env) {
  ...
  try {
    ...
    const closed = failClosed(env);  // default is fail-open: a dead API must not freeze the agent
    const reason = `sentinel unavailable (${err.message}) and the configured failure policy requires a pause`;
    ...
    else if (closed && event === "PermissionRequest") out = { hookSpecificOutput: { hookEventName: event, decision: { behavior: "deny", message: reason } } };
