...
export const CONFIG_FILE = join(homedir(), ".sentinel", "config.json");
...
export function readConfig(env = process.env) {
  try { return JSON.parse(readFileSync(env.SENTINEL_CONFIG ?? env.JEV_GUARD_CONFIG ?? CONFIG_FILE, "utf8")); } catch {
    if (env.SENTINEL_CONFIG || env.JEV_GUARD_CONFIG) return {};
    try { return JSON.parse(readFileSync(join(homedir(), ".jev-guard", "config.json"), "utf8")); } catch { return {}; }
  }
}
