...
Sentinel sends action arguments, recent user context, bounded trajectory state and scanned tool content to the selected backend: TypeSafe, Vercel AI Gateway, or OpenRouter Decisions. Bounded result excerpts are also retained in private local session state and shown on the local dashboard. Do not assume local redaction prevents sensitive data from reaching the model provider. The dashboard binds to loopback and applies same-origin checks; it is not designed for remote/multi-user hosting.
