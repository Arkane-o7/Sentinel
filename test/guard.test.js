...
const env = { JEV_API_KEY: "test", SENTINEL_MODE: "action-local" };
process.env.SENTINEL_MODE = "action-local";
...
test("acp proxy: rejects dangerous terminal/create, asks on medium, flags read content", async () => {
  ...
  assert.match(flagged.params.result.content, /^\[sentinel: .*canary/);
...
test("opencode plugin: throws on deny, rewrites flagged output, drives permission.ask", async () => {
  ...
  try {
    ...
    assert.match(out.output, /^\[sentinel: .*injection/);
...
test("install writes valid config for every target", async () => {
  ...
  const files = { claude: ".claude/settings.json", codex: ".codex/hooks.json", copilot: ".copilot/hooks/sentinel.json", gemini: ".gemini/settings.json",
    cursor: ".cursor/hooks.json", pi: ".pi/agent/settings.json", opencode: ".config/opencode/plugins/sentinel.js" };
  for (const [target, rel] of Object.entries(files)) {
    ...
    assert.ok(existsSync(join(home, rel)) && text.includes("sentinel"), target);
    if (rel.endsWith(".json")) {  // idempotent: exactly one sentinel entry per event, whatever the checkout path looks like
      const cfg = JSON.parse(text);
      for (const [ev, groups] of Object.entries(cfg.hooks ?? {})) assert.equal(groups.filter((g) => JSON.stringify(g).includes("sentinel")).length, 1, `${target} ${ev} duplicated`);
    }
  }
  const cursor = JSON.parse(readFileSync(join(home, files.cursor), "utf8"));
  assert.equal(cursor.hooks.beforeShellExecution.length, 1);
  assert.equal(cursor.hooks.preToolUse[0].matcher, ".*");
});
...
test("key: config file is read when env has no credentials", async () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "sentinel-key-"));
  ...
  const file = join(home, ".sentinel", "config.json");
...
test("context: the user's request lifts ask, flagged content turns a follow-up into deny, instruction files get reported", async () => {
  ...
  const { tmpdir } = await import("node:os");
  const { join } = await import("node:path");
  const home = mkdtempSync(join(tmpdir(), "sentinel-ctx-"));
...
test("scan-skills: sweeps instruction files, caches by hash, exits 2 when flagged", async () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "sentinel-skills-"));
